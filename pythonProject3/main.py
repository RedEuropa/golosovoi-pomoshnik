...
def run_person_through_social_nets_databases(*args: tuple):
    """
    Поиск человека по базе данных социальных сетей ВКонтакт
...
def say_time(*args: tuple):
    ...
    play_voice_assistant_speech(translator.get("Сейчас {0}").format(now.time()))  # Как милисикунды убрать не придумал


def who_are_u(*args: tuple):
    play_voice_assistant_speech(
        translator.get("Меня зовут Алиса, но в коде можно поменять мое имя, я была взята из бесплатного ресурса, "
                       "но дороботанна Барановым Максимом для колледжа МЦК ЧЭМК"))
...
config = {
    "intents": {
        ...
        "say_time": {
            "examples": ["скажи время", "какой час", "сколько времени",
                         "what is time", "tale me time"],
            "responses": say_time
        },
        "who_are_u": {
            "examples": ["кто ты", "расскажи о себе", "кто автор",
                         "who are you", "author"],
            "responses": who_are_u
        }
    },
